...
-- [2] -> blob ID					---> accessible as _mosaic_data_table[2 + 8*N] for N blobs
-- [3] -> blob age (milliseconds)
-- [4] -> blob center X
-- [5] -> blob center Y
-- [6] -> blob bounding rect X		---> accessible as _mosaic_data_table[6 + 8*N] for N blobs
...
function draw()
	...
	for j=0, numBlobs-1 do
		x = _mosaic_data_table[6 + 8*j]
		y = _mosaic_data_table[7 + 8*j]
		w = _mosaic_data_table[8 + 8*j]
		h = _mosaic_data_table[9 + 8*j]

		of.setColor(31,165,210)
		of.drawRectangle(x,y,w,h)

		center = of.Vec2f(_mosaic_data_table[4 + 8*j],_mosaic_data_table[5 + 8*j])

		of.setColor(255)
		of.drawBitmapString(string.format("%s:%s",_mosaic_data_table[2 + 8*j],_mosaic_data_table[3 + 8*j]),x,y-6)
	end
