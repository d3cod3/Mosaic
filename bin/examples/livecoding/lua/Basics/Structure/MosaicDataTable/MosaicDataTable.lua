...
function draw()
	...
	-- check _mosaic_data_table size changes
	if lastTableSize ~= tableSize then
		lastTableSize = tableSize
		of.log(of.LOG_NOTICE,string.format("_mosaic_data_table size: %i",tableSize))
	end
	-----------------------------------------

	-- draw something using _mosaic_data_table data
	of.setColor(255,255,255,10)
	if tableSize > 1 then
		of.pushMatrix()
		of.translate(OUTPUT_WIDTH/2,OUTPUT_HEIGHT/2,0)
		of.drawCircle(0,0,_mosaic_data_table[1]*OUTPUT_HEIGHT/2)
		of.popMatrix()
	end
